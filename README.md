...
- **Clipboard History** — Maintains a history of past clipboard snapshots. Press the clipboard history key (configurable) while an inventory is open to browse, select, or delete saved layouts with vanilla-style slot rendering and hover tooltips. Preview grids use authentic Minecraft slot sprites via `drawGuiTexture()` with items rendered proportionally using matrix-scaled transforms. Player inventory entries match the real vanilla inventory layout (armor column, offhand, main grid, hotbar with separator gap) with empty-slot placeholder sprites for unoccupied armor and offhand slots. Supports multi-select deletion (Shift+click for range, Ctrl/Cmd+click to toggle). Duplicate copies automatically replace older identical entries. Small container entries display with type-appropriate grid layouts: 3x3 for dispensers/droppers/crafters, 5x1 horizontal for hoppers, vertical input/fire/fuel for furnaces. Bundle entries use a dark-themed preview matching vanilla bundle tooltip styling.
